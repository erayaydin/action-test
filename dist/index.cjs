...
function validateInternalConfig(raw) {
  ...
  if (typeof raw.file_marker !== "string") {
    throw new Error('"internal.file_marker" must be a string');
  }
...
function loadConfig(configPath) {
  ...
  if (mode === "bundled") {
    if (typeof raw.bundled_file !== "string" || raw.bundled_file.length === 0) {
      throw new Error("Bundled mode requires a non-empty `bundled_file` string");
    }
  }
  ...
  if (mode === "bundled" && typeof raw.bundled_file === "string") {
    config.bundled_file = raw.bundled_file;
  }
  return config;
}
...
async function resolveRefs(entrypoint, sourceRoot) {
  ...
  const normalizedEntry = path2.relative(sourceRoot, path2.resolve(sourceRoot, entrypoint));
...
async function walkFile(relPath, sourceRoot, visited) {
  ...
  visited.add(relPath);
  const absPath = path2.resolve(sourceRoot, relPath);
  let content;
  try {
  ...
  const ext2 = path2.extname(relPath).toLowerCase();
  ...
  const currentDir = path2.dirname(relPath);
...
function collectRefs(node, currentDir, refs) {
  ...
  for (const [key, value] of Object.entries(node)) {
    if (key === "$ref" && typeof value === "string") {
      ...
      const resolved = path2.normalize(path2.join(currentDir, filePart));
...
// src/filter.ts
var HTTP_METHODS = /* @__PURE__ */ new Set(["get", "post", "put", "delete", "patch", "options", "head", "trace"]);
...
function filterFiles(files, reachableFiles, config) {
  ...
  for (const [filePath, content] of filtered) {
    ...
    const currentDir = path4.dirname(filePath);
...
function cleanDanglingRefs(node, excludedFiles, currentDir) {
  if (Array.isArray(node)) {
    ...
    for (const item of node) {
      ...
      warnings2.push(...itemResult.warnings);
...
function collectFiles(dir, baseDir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      results.push(path5.relative(baseDir, fullPath).split(path5.sep).join("/"));
    }
  }
  return results;
}
function computeDiff(newFiles, targetRoot, managedDirs) {
  ...
  for (const [targetPath, newContent] of newFiles) {
    const absPath = path5.resolve(targetRoot, targetPath);
    if (!fs3.existsSync(absPath)) {
      added.push(targetPath);
      continue;
    }
    const existingContent = fs3.readFileSync(absPath, "utf-8");
  ...
  for (const managedDir of managedDirs) {
    const absDir = path5.resolve(targetRoot, managedDir);
...
function writeFiles(files, targetRoot) {
  for (const [targetPath, content] of files) {
    const absPath = path6.resolve(targetRoot, targetPath);
...
function deleteFiles(filePaths, targetRoot) {
  for (const filePath of filePaths) {
    const absPath = path6.resolve(targetRoot, filePath);
  ...
  for (const dir of sortedDirs) {
    const absDir = path6.resolve(targetRoot, dir);
    if (fs4.existsSync(absDir)) {
      const entries = fs4.readdirSync(absDir);
      if (entries.length === 0) {
        fs4.rmdirSync(absDir);
      }
    }
  }
}
...
var COMMENT_TAG_WARNING = "<!-- openapi-sync-warning -->";
var COMMENT_TAG_MERGED = "<!-- openapi-sync-merged -->";
var WARNING_LABEL = "Source PR Not Merged";
...
async function syncMultiFile(config, sourceRoot) {
  ...
  for (const filePath of reachableFiles) {
    const absPath = path7.resolve(sourceRoot, filePath);
...
function syncBundled(config, sourceRoot) {
  ...
  const absPath = path7.resolve(sourceRoot, bundledFile);
